...
@section('title', 'Running Pace Calculator - Free Online')
@section('meta_description', 'Free Calculate running pace per mile or km. Step-by-step solutions and formulas included.')
@section('canonical', config('site.url') . '/sports/running-pace-calculator')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <p class="text-lg text-gray-600 mb-8">Calculate running pace per mile or km.</p>

    <div x-data="runningPaceCalculator()" class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-12">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            ...
            <div>
                ...
                <input type="number" x-model.number="hours" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="0">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Minutes</label>
                <input type="number" x-model.number="minutes" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="40">
            </div>
        </div>
    ...
    <div class="max-w-3xl">
        ...
        <p class="text-gray-600 leading-relaxed mb-4">Calculate running pace per mile or km. This calculator provides instant results with step-by-step explanations to help you understand the calculation process.</p>

        <h2 class="text-2xl font-bold text-gray-900 mb-4">Formula</h2>
        <div class="bg-gray-50 rounded-lg p-4 mb-4 text-center">
            <p class="formula-block text-lg font-mono">Pace = Total Time / Distance</p>
        </div>
...
function runningPaceCalculator() {
    return {
        ...
        calculate() {
            try {
                let totalMin = this.hours * 60 + this.minutes;
let pace = totalMin / this.distance;
let paceMin = Math.floor(pace);
let paceSec = Math.round((pace - paceMin) * 60);
this.result = paceMin + ':' + String(paceSec).padStart(2, '0') + ' /mile';
this.steps = 'Total time: ' + totalMin + ' min\nPace: ' + this.result + '\nSpeed: ' + (this.distance / (totalMin/60)).toFixed(1) + ' mph';
