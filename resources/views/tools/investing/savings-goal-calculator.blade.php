...
@section('title', 'Savings Goal Calculator - Free Online')
@section('meta_description', 'Free Calculate monthly savings needed to reach a goal. Step-by-step solutions and formulas included.')
@section('canonical', config('site.url') . '/investing/savings-goal-calculator')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <p class="text-lg text-gray-600 mb-8">Calculate monthly savings needed to reach a goal.</p>

    <div x-data="savingsGoalCalculator()" class="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 mb-12">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Savings Goal ($)</label>
                <input type="number" x-model.number="goal" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="50000">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Current Savings ($)</label>
                <input type="number" x-model.number="current" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="5000">
            </div>
            <div>
    ...
    <div class="max-w-3xl">
        ...
        <p class="text-gray-600 leading-relaxed mb-4">Calculate monthly savings needed to reach a goal. This calculator provides instant results with step-by-step explanations to help you understand the calculation process.</p>

        <h2 class="text-2xl font-bold text-gray-900 mb-4">Formula</h2>
        <div class="bg-gray-50 rounded-lg p-4 mb-4 text-center">
            <p class="formula-block text-lg font-mono">PMT = (FV − PV(1+r)^n) × r / ((1+r)^n − 1)</p>
        </div>
...
let n = this.months;
let fvCurrent = this.current * Math.pow(1+r,n);
let remaining = this.goal - fvCurrent;
let monthly = remaining * r / (Math.pow(1+r,n) - 1);
this.result = '$' + monthly.toFixed(2) + '/month';
this.steps = 'Goal: $' + this.goal.toLocaleString() + '\nCurrent will grow to: $' + fvCurrent.toFixed(2) + '\nMonthly savings needed: ' + this.result;
